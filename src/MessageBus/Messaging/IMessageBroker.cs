...
namespace MessageBus.Messaging
{
    public interface IMessageBroker : IDisposable
    {
        ...
        /// </summary>
        /// <remarks>With the parameter <paramref name="options"/>, you can pass configuration options
        /// which describe the command handler behavior.</remarks>
        /// <exception cref="InvalidOperationException">Is thrown if the provided <paramref name="topic"/>
        /// already exists but it isn't a command channel.</exception>
        ISubscribable Commands(TopicName topic, ISubscriptionOptions? options);

        /// <summary>
        /// Publishes the provided <paramref name="message"/> to all event
        /// channels specified in <paramref name="topics"/>. If a channel
        /// does not exist, it will get skipped silently.
        /// </summary>
        Task PublishEvent<T>(T message, IReadOnlyList<TopicName> topics);

        /// <summary>
        /// Publishes the provided <paramref name="message"/> to all command
        /// channels specified in <paramref name="topics"/>. If a channel
        /// does not exist, it will get skipped silently.
        /// </summary>
        Task PublishCommand<T>(T message, IReadOnlyList<TopicName> topics);
    }
    ...
    public static class IMessageBrokerExtensions
    {
        ...
        public static ISubscribable Commands(this IMessageBroker broker, TopicName topic)
        {
            return broker.Commands(topic, null);
        }

        public static Task PublishEvent<T>(this IMessageBroker broker, T message, TopicName topic)
        {
            return broker.PublishEvent(message, new[] { topic });
        }

        public static Task PublishEvent<T>(this IMessageBroker broker, T message, TopicName topic, params TopicName[] otherTopics)
{
            return broker.PublishEvent(message, new[] { topic }.Concat(otherTopics).ToArray());
        }

        public static Task PublishCommand<T>(this IMessageBroker broker, T message, TopicName topic)
        {
            return broker.PublishCommand(message, new[] { topic });
        }

        public static Task PublishCommand<T>(this IMessageBroker broker, T message, TopicName topic, params TopicName[] otherTopics)
        {
            return broker.PublishCommand(message, new[] { topic }.Concat(otherTopics).ToArray());
        }
    }
}
