...
namespace MessageBus.Examples.Broker.Events
{
    class Program
    {
        ...
        {
            broker.Events(MyChannel).Subscribe<MyChannelMessage>(m => HandleMyChannelMessage(m, subscriberName, onComplete));
        }
        ...
        {
            await broker.PublishEvent(new MyChannelMessage("Hello World!"), MyChannel);
            foreach (var id in Enumerable.Range(0, 10))
                await broker.PublishEvent(new MyChannelMessage($"Message {id}"), MyChannel);
            await Task.Delay(2000);
            await broker.PublishEvent(new MyChannelMessage("Hello World after 2 sec."), MyChannel);
        }
    }
