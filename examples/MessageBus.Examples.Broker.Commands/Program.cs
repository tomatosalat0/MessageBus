...
namespace MessageBus.Examples.Broker.Commands
{
    class Program
    {
        ...
        {
            await broker.PublishCommand(new MyChannelMessage("Hello World!"), MyChannel);
            foreach (var id in Enumerable.Range(0, 10))
                await broker.PublishCommand(new MyChannelMessage($"Message {id}"), MyChannel);
            await Task.Delay(2000);
            await broker.PublishCommand(new MyChannelMessage("Hello World after 2 sec."), MyChannel);
        }
    }
