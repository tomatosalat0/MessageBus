...
namespace MessageBus.Tests.UnitTests
{
    ...
    public class MessageBusQueriesTests
    {
        ...
        public async Task QueryHandlerIsCalledForQueries()
        {
            ...
            MessageQueryResult result = await bus.FireQuery<QueryA, MessageQueryResult>(query, TimeSpan.FromSeconds(2));
        ...
        public async Task ExceptionsInQueryWillGetThrownWithCustomExceptionOnReceive()
        {
            ...
            await Assert.ThrowsExceptionAsync<MessageOperationFailedException>(async () =>
                await bus.FireQuery<QueryA, MessageQueryResult>(query, TimeSpan.FromSeconds(2))
            );
        }
