...
namespace MessageBus.Tests.UnitTests
{
    ...
    public class MessageBusRpcTests
    {
        ...
        public async Task RpcHandlerIsCalledForRpcs()
        {
            ...
            ToUpperCaseResult result = await bus.FireRpc<ToUpperCase, ToUpperCaseResult>(query, TimeSpan.FromSeconds(2));
        ...
        [TestMethod]
        public async Task ExceptionsInQueryWillGetThrownWithCustomExceptionOnReceive()
        {
            using IMessageBus bus = new MessageBrokerMessageBus(MemoryMessageBrokerBuilder.InProcessBroker(), NoExceptionNotification.Instance);
            ...
            await Assert.ThrowsExceptionAsync<MessageOperationFailedException>(async () =>
                await bus.FireRpc<ToUpperCase, ToUpperCaseResult>(query, TimeSpan.FromSeconds(2))
            );
        }
